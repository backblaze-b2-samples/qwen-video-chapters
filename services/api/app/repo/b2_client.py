...
def get_s3_client():
    # endpoint_url is derived from B2_REGION; region_name is passed explicitly
    # so SigV4 signing uses the right region. No hardcoded region strings.
    return boto3.client(
        "s3",
        ...
        config=Config(
            ...
            user_agent_extra="b2ai-qwen-video-chapters",
        ),
    )
...
def put_bytes(key: str, data: bytes, content_type: str) -> None:
    """Upload raw bytes (thumbnails, meta/index JSON). RuntimeError on failure."""
    client = get_s3_client()
    try:
        client.put_object(
            Bucket=settings.b2_bucket_name,
...
def get_bytes(key: str) -> bytes | None:
    """Read an object's body. Returns None if the key does not exist."""
    client = get_s3_client()
    try:
...
def download_to_path(key: str, dest_path: str) -> None:
    """Stream an object's body from B2 to a local file (e.g. for ffmpeg).
...
def list_keys(prefix: str, max_keys: int = 1000) -> list[str]:
    """Return raw object keys under a prefix (paginated). RuntimeError on error."""
    client = get_s3_client()
    keys: list[str] = []
    kwargs: dict = {
        "Bucket": settings.b2_bucket_name,
...
def delete_prefix(prefix: str) -> int:
    """Delete every object under a prefix (paginated). Returns count deleted.

    The caller is responsible for passing a scoped, non-empty prefix — this
    must never be called with the bucket root. Raises RuntimeError on failure.
    """
    if not prefix:
