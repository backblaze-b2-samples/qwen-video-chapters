...
import { useLibraryActivity } from "@/lib/queries";
...
function ChapterChartSkeleton() {
  return (
    ...
    >
      <span className="sr-only">Loading chapter activity</span>
...
export function UploadChart() {
  const { data: activity, isLoading, error, refetch } = useLibraryActivity();
  ...
  return (
    <Card>
      <CardHeader className="border-b border-border py-4 px-5">
        <CardTitle className="card-title">Chapters Generated</CardTitle>
      ...
      <CardContent className="p-5">
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="Chapterize a video to see generation trends here."
          />
        ) : (
            ...
            <BarChart data={data} margin={{ top: 8, right: 4, left: -16, bottom: 0 }}>
              <defs>
                <linearGradient id="chapters-fill" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="var(--color-chapters)" stopOpacity={0.95} />
                  <stop offset="100%" stopColor="var(--color-chapters)" stopOpacity={0.55} />
                </linearGradient>
              </defs>
              <CartesianGrid
                vertical={false}
