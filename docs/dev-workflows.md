...
## ML / media stack (for chapter generation)

The core `requirements.txt` is kept light so the app, structural tests and lint
install fast and stay green WITHOUT the heavy stack. The torch / transformers /
qwen-vl-utils / accelerate / imageio-ffmpeg pins live in a separate file:

```bash
cd services/api && .venv/bin/pip install -r requirements-ml.txt
```

- **Lazy imports.** `repo/qwen_chapters.py` and `repo/keyframes.py` import the
  heavy modules inside functions only — never at module top. `from main import
  app`, `pnpm check:structure`, `pnpm test:api`, `pnpm lint` and `pnpm build`
  all pass without `requirements-ml.txt` installed.
  `tests/test_chapters_lazy.py` asserts no torch import at module load — keep it.
- **First run downloads the model** (`Qwen/Qwen2.5-VL-3B-Instruct`, a few GB)
  from the Hugging Face Hub, then caches it.
- **Device autodetect**: CUDA → Apple MPS → CPU (CPU default). Override with
  `QWEN_DEVICE` (`cpu` / `cuda` / `mps`). Qwen2.5-VL MPS support is partial — the
  adapter falls back to CPU automatically if loading on MPS raises.
- **Model override**: `QWEN_MODEL_ID`.
- **Demo frame cap**: `MAX_KEYFRAMES` (default 16) keeps a CPU demo feasible.
  Production would lift this and move chapterize behind a job queue + GPU.
- **ffmpeg**: always the bundled `imageio_ffmpeg.get_ffmpeg_exe()` binary, never
  a bare `ffmpeg` (Homebrew's is slim). Keyframe extraction needs no libass.
...
- E2E: `pnpm test:e2e` (run `pnpm --filter @qwen-video-chapters/web exec playwright install chromium` once first)
