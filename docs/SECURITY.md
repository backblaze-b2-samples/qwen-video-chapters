...
Security principles and implementation for qwen-video-chapters.
...
- **Client -> B2**: Presigned URLs for download and video/thumbnail playback (10-min expiry)
- **Scoped deletes**: deleting a Library Video removes objects only under that video's `library/source|thumbs|meta` prefixes — never the bucket root.
